...
const MODULES = [
  'reservations', 'payments', 'invoices', 'guests', 'housekeeping',
  'maintenance', 'expenses', 'staff', 'room_list', 'rooms', 'auth', 'settings',
]

const MODULE_LABELS: Record<string, string> = {
  reservations: 'Reservations', payments: 'Payments', invoices: 'Invoices',
  guests: 'Guests', housekeeping: 'Housekeeping', maintenance: 'Maintenance',
  expenses: 'Expenses', staff: 'Staff', room_list: 'Room List', rooms: 'Rooms',
  auth: 'Auth', settings: 'Settings',
}

const MODULE_COLORS: Record<string, 'info' | 'success' | 'warning' | 'danger' | 'default' | 'gold'> = {
  reservations: 'info', payments: 'success', invoices: 'gold', guests: 'warning',
  housekeeping: 'default', maintenance: 'danger', expenses: 'info', staff: 'default',
  room_list: 'default', rooms: 'warning', auth: 'default', settings: 'default',
}

const MODULE_ICONS: Record<string, LucideIcon> = {
  reservations: BedDouble, payments: CreditCard, invoices: ReceiptText,
  guests: UserRound, housekeeping: Sparkles, maintenance: Wrench,
  expenses: Wallet, staff: Users, room_list: DoorOpen, rooms: DoorOpen,
  auth: Lock, settings: Settings,
}
...
function moduleBorderColor(module: string): string {
  switch (MODULE_COLORS[module] ?? 'default') {
...
const ACTIONS = [
  'created', 'updated', 'deleted', 'checked_in', 'checked_out', 'cancelled',
  'marked_no_show', 'extended_stay', 'status_changed', 'assigned',
  'flagged_overdue', 'cleared_overdue', 'notified_overdue', 'late_checkout',
]

const ACTION_LABELS: Record<string, string> = {
  created: 'Created', updated: 'Updated', deleted: 'Deleted',
  checked_in: 'Checked In', checked_out: 'Checked Out', cancelled: 'Cancelled',
  marked_no_show: 'No Show', extended_stay: 'Extended Stay',
  status_changed: 'Status Changed', assigned: 'Assigned',
  flagged_overdue: 'Flagged Overdue', cleared_overdue: 'Cleared Overdue',
  notified_overdue: 'Overdue Notified', late_checkout: 'Late Check-Out',
}

const ACTION_COLORS: Record<string, 'info' | 'success' | 'warning' | 'danger' | 'default'> = {
  created: 'success', updated: 'info', deleted: 'danger',
  checked_in: 'success', checked_out: 'warning', cancelled: 'danger',
  marked_no_show: 'danger', extended_stay: 'info', status_changed: 'warning',
  assigned: 'info', flagged_overdue: 'danger', cleared_overdue: 'success',
  notified_overdue: 'warning', late_checkout: 'warning',
}

// ─── Diff display helpers ────────────────────────────────────────────────────

const FIELD_LABELS: Record<string, string> = {
  status: 'Status', payment_status: 'Payment Status', cleaning_status: 'Cleaning Status',
  price_per_night: 'Price / Night', total_amount: 'Total Amount', paid_amount: 'Paid Amount',
  due_amount: 'Balance Due', discount_percent: 'Discount %', tax_percent: 'Tax %',
  room_id: 'Room', guest_id: 'Guest', check_in: 'Check-in', check_out: 'Check-out',
  is_active: 'Active', is_blacklisted: 'Blacklisted', is_overdue: 'Overdue',
  role_id: 'Role', notes: 'Notes',
}
...
function formatDiffValue(value: unknown, field?: string): string {
  ...
  if (typeof value === 'number' && field && CURRENCY_FIELDS.has(field)) return formatCurrency(value)
...
function splitDescription(description: string): DescriptionToken[] {
  ...
  while ((match = DESCRIPTION_TOKEN_PATTERN.exec(description)) !== null) {
    ...
    if (match[1]) tokens.push({ type: 'reservation', value: match[1] })
    else if (match[2]) tokens.push({ type: 'id', value: match[2] })
    else if (match[3]) tokens.push({ type: 'currency', value: match[3] })
    else if (match[4]) tokens.push({ type: 'email', value: match[4] })
    ...
    <p className="flex flex-wrap items-baseline gap-x-1.5 gap-y-1 whitespace-pre-wrap break-words text-sm leading-relaxed text-foreground">
      {tokens.map((token, i) => {
        if (token.type === 'reservation') {
          return (
            <span key={i} className="inline-flex items-center rounded-md bg-gold/15 px-1.5 py-0.5 font-mono text-xs font-semibold text-gold-dark">
              {token.value}
            </span>
          )
        }
        if (token.type === 'id') {
          return (
            <span key={i} className="inline-flex items-center rounded-md bg-bg px-1.5 py-0.5 font-mono text-xs font-medium text-primary">
              {token.value}
            </span>
          )
        }
        if (token.type === 'currency') {
          return <span key={i} className="font-semibold text-success">{token.value}</span>
        }
        if (token.type === 'email') {
          return <span key={i} className="font-mono text-xs text-muted">{token.value}</span>
        }
...
function groupLogsByDate(logs: ActivityLog[]): DateGroup[] {
  ...
  return Array.from(map.entries()).map(([key, items]) => ({
    key,
    label: formatDateGroupLabel(items[0].created_at),
    items,
  }))
}
...
function ActivityFeedRow({
  ...
  const Icon = MODULE_ICONS[log.module] ?? UserRound
  ...
  return (
    <button
      ...
      className={`group flex w-full items-start gap-3 border-l-[3px] bg-transparent px-4 py-3 text-left transition-colors hover:bg-bg/50 ${moduleBorderColor(log.module)}`}
    >
      <div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg ${moduleTileClass(log.module)}`}>
      ...
      <div className="min-w-0 flex-1">
        <div className="mb-0.5 flex flex-wrap items-center gap-1.5">
          <span className="text-sm font-semibold text-foreground">
            {isGuestRow(log) ? 'Guest' : (log.user?.name ?? 'Unknown')}
          </span>
          <Badge variant={ACTION_COLORS[log.action] ?? 'default'}>
            {ACTION_LABELS[log.action] ?? log.action}
          </Badge>
          <Badge variant={MODULE_COLORS[log.module] ?? 'default'}>
            {MODULE_LABELS[log.module] ?? log.module}
          </Badge>
        </div>
        <DescriptionText description={log.description} />
      </div>

      <div className="flex shrink-0 flex-col items-end gap-1">
        <span className="whitespace-nowrap text-xs text-muted">{time}</span>
        <span className="whitespace-nowrap text-xs text-muted">
          {isGuestRow(log) ? 'Guest' : 'Staff'}
        </span>
        <Eye className="mt-1 h-4 w-4 text-muted/0 transition-colors group-hover:text-muted" />
      </div>
    </button>
  )
}
...
function ActivityFeed({
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center rounded-xl border border-border bg-card py-12">
        ...
        <p className="mb-4 text-sm text-muted">Failed to load activity logs</p>
    ...
    return (
      <div className="divide-y divide-border rounded-xl border border-border bg-card">
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={`skeleton-${i}`} className="flex items-start gap-3 border-l-[3px] border-l-border/30 px-4 py-3">
            <div className="h-9 w-9 animate-pulse rounded-lg bg-bg" />
            <div className="flex-1 space-y-2">
              <div className="flex items-center gap-2">
                <div className="h-4 w-24 animate-pulse rounded bg-bg" />
                <div className="h-5 w-14 animate-pulse rounded-full bg-bg" />
            ...
            <div className="space-y-1.5">
              ...
              <div className="h-3 w-10 animate-pulse rounded bg-bg" />
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-border bg-card">
      {groups.map((group) => (
        <div key={group.key}>
          <div className="sticky top-0 z-10 border-b border-border bg-bg/80 px-4 py-2 backdrop-blur-sm">
          ...
          <div className="divide-y divide-border">
            {group.items.map((log) => (
              <ActivityFeedRow key={log.id} log={log} onSelect={onSelect} />
            ))}
          </div>
        </div>
      ))}
...
export default function ActivityLogsPage() {
  ...
  const hasActiveFilters = Boolean(search || scope || moduleFilter || actionFilter || staffFilter)
            ...
            <div className="mb-4 flex flex-wrap items-center gap-2 text-sm text-muted">
              ...
              {search && (
                <Badge variant="secondary" className="gap-1">
                  Search: {search}
                  <Button variant="ghost" size="sm" className="h-5 w-5 p-0" onClick={() => resetPage(setSearch)('')}>
                    <X className="h-3 w-3" />
                  </Button>
                </Badge>
              )}
              {scope && (
                <Badge variant="secondary" className="gap-1">
                  Actor: {scope === 'staff' ? 'Staff' : 'Guest'}
                  <Button variant="ghost" size="sm" className="h-5 w-5 p-0" onClick={() => { setScope(''); setPage(1) }}>
                    <X className="h-3 w-3" />
                  </Button>
                </Badge>
              )}
              {moduleFilter && (
                <Badge variant="secondary" className="gap-1">
                  Module: {MODULE_LABELS[moduleFilter] ?? moduleFilter}
                  <Button variant="ghost" size="sm" className="h-5 w-5 p-0" onClick={() => { setModuleFilter(''); setPage(1) }}>
                    <X className="h-3 w-3" />
                  </Button>
                </Badge>
              )}
              {actionFilter && (
                <Badge variant="secondary" className="gap-1">
                  ...
                  <Button variant="ghost" size="sm" className="h-5 w-5 p-0" onClick={() => { setActionFilter(''); setPage(1) }}>
                    <X className="h-3 w-3" />
                  </Button>
                </Badge>
              )}
              {staffFilter && (
                <Badge variant="secondary" className="gap-1">
                  Staff: {staffOptions?.find((s) => String(s.id) === staffFilter)?.name ?? staffFilter}
                  <Button variant="ghost" size="sm" className="h-5 w-5 p-0" onClick={() => { setStaffFilter(''); setPage(1) }}>
                    <X className="h-3 w-3" />
                  </Button>
                </Badge>
              )}
                ...
                <div className="rounded-xl bg-bg p-3">
                  ...
                  <p className="text-sm font-semibold text-foreground">
                    {(() => {
                      ...
                      return `${formatDateDisplay(selected.created_at, 'long')} · ${time}`
                    })()}
                  </p>
                </div>
                <div className="rounded-xl bg-bg p-3">
